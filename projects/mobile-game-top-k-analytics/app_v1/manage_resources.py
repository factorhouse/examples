        ...
        for topic, future in result_dict.items():
            try:
                ...
                if e.args[0].code() == KafkaError.TOPIC_ALREADY_EXISTS:
                    logging.warning(f"Topic exists: {topic}")
                else:
        ...
        for topic, future in result_dict.items():
            try:
                ...
                if e.args[0].code() == KafkaError.UNKNOWN_TOPIC_OR_PART:
                    logging.warning(f"Topic not existing: {topic}")
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A utility script to create or delete Kafka topics required for the mobile game leaderboard V1."
    )
    parser.add_argument("--action", choices=["create", "delete"], required=True, help="Action to perform on the topics")
    parser.add_argument(
        "--topic-names", type=str, default="top-teams,top-players,hot-streaks,team-mvps",
        help="Comma-separated Kafka topic names Defaults to: top-teams,top-players,hot-streaks,team-mvps",
    )
