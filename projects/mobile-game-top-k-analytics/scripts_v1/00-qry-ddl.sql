...
-- // Source table
DROP TABLE IF EXISTS user_scores_v1;
CREATE TABLE user_scores_v1 (
  `user_id`           STRING,
  `team_id`           STRING,
  `team_name`         STRING,
  `score`             INT,
...
-- // Top teams table
DROP TABLE IF EXISTS top_teams_v1;
CREATE TABLE top_teams_v1 (
  rnk         BIGINT,
  team_id     STRING,
  team_name   STRING,
  total_score BIGINT,
...
-- // Top players table
DROP TABLE IF EXISTS top_players_v1;
CREATE TABLE top_players_v1 (
  rnk         BIGINT,
  user_id     STRING,
  team_name   STRING,
  total_score BIGINT,
...
-- // Hot streaks table
DROP TABLE IF EXISTS hot_streaks_v1;
CREATE TABLE hot_streaks_v1 (
  rnk             BIGINT,
  user_id         STRING,
  ...
  'key.avro-confluent.basic-auth.credentials-source' = 'USER_INFO',
  'key.avro-confluent.basic-auth.user-info' = 'admin:admin',
  'key.avro-confluent.schema-registry.subject' = 'hot-streaks-key',

  'value.format' = 'avro-confluent',
  'value.avro-confluent.schema-registry.url' = 'http://schema:8081',
  'value.avro-confluent.basic-auth.credentials-source' = 'USER_INFO',
  'value.avro-confluent.basic-auth.user-info' = 'admin:admin',
  'value.avro-confluent.schema-registry.subject' = 'hot-streaks-value',

  'sink.parallelism' = '3'
);

-- // Team MVP table
DROP TABLE IF EXISTS team_mvps_v1;
CREATE TABLE team_mvps_v1 (
  rnk             BIGINT,
  user_id         STRING,
  team_name       STRING,
  player_total    BIGINT,
